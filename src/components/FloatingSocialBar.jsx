...
export default function ProfessionalContactMenu() {
  ...
  const handleMainButtonClick = () => {
    // Toggle for mobile devices - clicking opens/closes
    setIsOpen(!isOpen);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={`absolute inset-0 rounded-full bg-[#8BE31F] 
          animate-ping opacity-20 ${isOpen ? 'hidden' : 'block'}`}></div>
        ...
        <div className="absolute inset-0 rounded-full overflow-hidden">
          <div className="absolute inset-0 rounded-full bg-white opacity-0 
            ...
            active:opacity-30 active:scale-150"></div>
        </div>
      </button>
    </div>
  );
