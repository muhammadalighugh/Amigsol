...
import { ArrowRight, Code, Zap, Globe, Star, Handshake, ChevronDown } from "lucide-react";
...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    // when word complete pause then start deleting
    if (subIndex === words[index].length + 1 && !reverse) {
      setTimeout(() => setReverse(true), 1000);
      return;
    }

    // when word deleted, move to next word
      ...
      <div className="relative z-10 mx-auto px-4 sm:px-6 lg:px-20 min-h-screen flex flex-col">
        <div className="flex-1 flex flex-col lg:flex-row items-center justify-center pt-5 pb-16 gap-1">
          ...
          <div className="max-w-3xl text-center lg:text-left">
            <div className={`inline-flex items-center px-4 py-2 rounded-full bg-[#8BE31F] border border-[#8BE31F]/20 text-[#1E1E1E] text-sm font-medium mb-8 backdrop-blur-sm transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <p className={`text-lg sm:text-xl lg:text-2xl text-gray-700 dark:text-gray-300 mb-12 max-w-3xl leading-relaxed transition-all duration-1000 delay-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            {/* Buttons */}
            <div className={`flex flex-col sm:flex-row gap-10 transition-all duration-1000 delay-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className="w-full flex justify-center lg:justify-end max-w-lg lg:max-w-4xl transition-all duration-1000 delay-900 sm:my-0">
            {animationData && (
              <Lottie
        ...
        {/* Features */}
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-8 mb-10 transition-all duration-1000 delay-1100 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            >
              ...
              <div className="relative z-10 text-center">
                <div className="inline-flex items-center justify-center w-16 h-16 bg-[#8BE31F]/10 rounded-2xl mb-6 group-hover:bg-[#8BE31F]/20 group-hover:scale-110 transition-all duration-300">
                  <feature.icon className="h-8 w-8 text-[#8BE31F] group-hover:scale-110 transition-transform duration-300" />
                </div>
