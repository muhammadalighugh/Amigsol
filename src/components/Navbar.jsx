    ...
    <>
      ...
      >
        ...
        <div className={`fixed inset-0 z-40 lg:hidden transition-all duration-500 ${isMobileMenuOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}>
          <div
            className={`fixed inset-0 bg-black/60 backdrop-blur-sm transition-all duration-500 ${isMobileMenuOpen ? "opacity-100" : "opacity-0"}`}
            onClick={() => setIsMobileMenuOpen(false)}
          />
          <div className={`fixed inset-y-0 left-0 w-80 max-w-[85vw] bg-black border-r border-gray-800/80 z-50 transform transition-all duration-500 ease-out shadow-2xl ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                item.dropdown ? (
                  <div key={item.label} className="space-y-1">
                    ...
                    >
                      ...
                      <span className="relative z-10 group-hover:text-[#8BE31F] transition-colors duration-300">{item.label}</span>
                      <ChevronDown className={`relative z-10 h-4 w-4 transition-all duration-300 group-hover:text-[#8BE31F] ${activeDropdown === index ? "rotate-180" : ""}`} />
                    </button>
                    <div className={`overflow-hidden transition-all duration-300 ${activeDropdown === index ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
                      <div className="pl-4 pr-2 pb-2 space-y-3">
                        {item.dropdown.map((dropdownItem) => (
                          <div key={dropdownItem.label} className="space-y-2">
                            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-3">{dropdownItem.label}</p>
